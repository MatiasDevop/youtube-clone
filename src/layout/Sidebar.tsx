...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <aside
        className={`w-56 lg:sticky absolute top-0 overflow-y-auto scrollbar-hidden pb-4 flex-col gap-2 px-2 ${isLargeOpen ? "lg:flex" : "lg:hidden"} ${isSmallOpen ? "flex z-[999] bg-white max-h-screen" : "hidden"}`}
      >
        ...
        <LargeSidebarSection title="Subcriptions">
          {subscriptions.map((subcription) => (
            <LargeSidebarItem
              key={subcription.id}
              IconOrImgUrl={subcription.imgUrl}
              title={subcription.channelName}
              url={`/@${subcription.id}`}
            />
          ))}
        </LargeSidebarSection>
